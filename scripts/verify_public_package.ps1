...
function Test-VersionConsistency {
    $packageText = Read-Text "package.yaml"
    $readmeText = Read-Text "README.md"
    $changelogText = Read-Text "CHANGELOG.md"
    $match = [regex]::Match($packageText, "(?m)^version:\s*([0-9]+\.[0-9]+\.[0-9]+)\s*$")
    if (-not $match.Success) {
        Add-Error "package.yaml missing semantic version"
        return
    }
    $version = $match.Groups[1].Value
    if ($readmeText.IndexOf("パッケージ版: ``$version``", [System.StringComparison]::Ordinal) -lt 0) {
        Add-Error "README.md package version does not match package.yaml: $version"
    } else {
        Add-Checked "README.md package version matches package.yaml: $version"
    }
    if ($changelogText.IndexOf("## $version", [System.StringComparison]::Ordinal) -lt 0) {
        Add-Error "CHANGELOG.md missing current package version: $version"
    } else {
        Add-Checked "CHANGELOG.md contains current package version: $version"
    }
}
...
Test-Contains "README.md" "scripts/check_version_bump.py"
Test-Contains "README.md" "base branch より上がっていること"
Test-Contains "package.yaml" "scripts/run_local_draft_qa_proof.py"
Test-Contains "package.yaml" "scripts/check_version_bump.py"
Test-Contains ".github/workflows/test.yml" "python scripts/check_version_bump.py"
...
Test-Contains "README.md" "scripts/fetch_note_body.js"
Test-Contains "package.yaml" "scripts/fetch_note_body.js"
...
$editorObservationPath = Join-Path ([System.IO.Path]::GetTempPath()) "note-editor-prepublish-observation.json"
...
$qaProofPath = "data/local_draft_qa_stop_before_publish_evidence.json"
$qaProofFullPath = Join-Path $root $qaProofPath
$qaProofTempPath = Join-Path ([System.IO.Path]::GetTempPath()) "note-publishing-suite-local-draft-qa-proof.json"
Test-RequiredFile $qaProofPath
Push-Location $root
try {
    $qaProofOutput = & python "scripts/run_local_draft_qa_proof.py" "--output" $qaProofTempPath "--json" 2>&1
} finally {
    Pop-Location
    Remove-Item -LiteralPath $qaProofTempPath -ErrorAction SilentlyContinue
}
...
if ($qaProofExitCode -ne 0) {
    ...
    try {
        ...
        if ($qaProofResult.overall -ne "stopped_before_publish") {
            ...
            Add-Checked "local draft QA proof stopped before publish with evidence: $qaProofPath"
        }
    } catch {
